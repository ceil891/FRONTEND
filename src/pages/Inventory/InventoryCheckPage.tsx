...
import {
  ...
  FilterAlt as FilterIcon, Visibility as ViewIcon, CheckCircle as BalanceIcon
} from '@mui/icons-material';

const mockChecks = [
  { no: 1, maPhieu: 'PK260301', ngayKiem: '06/03/2026', kho: 'Kho Hà Nội', slLech: -12, giaTriLech: -1500000, nguoiKiem: 'Thành Đào', trangThai: 'Đã cân bằng' },
  { no: 2, maPhieu: 'PK260302', ngayKiem: '06/03/2026', kho: 'Kho Hồ Chí Minh', slLech: +5, giaTriLech: 450000, nguoiKiem: 'Sale 01', trangThai: 'Đang kiểm' },
];
      ...
      <Card sx={{ borderRadius: 2, boxShadow: '0 2px 10px rgba(0,0,0,0.05)', border: 'none' }}>
        <CardContent sx={{ p: 0, '&:last-child': { pb: 0 } }}>
          ...
          <Box sx={{ p: 1.5, display: 'flex', flexWrap: 'wrap', gap: 0.5, borderBottom: '1px solid #f1f5f9', alignItems: 'center' }}>
            ...
            />
            <Button size="small" variant="contained" startIcon={<AddIcon />} sx={{ bgcolor: '#00a65a', '&:hover': { bgcolor: '#008d4c' }, textTransform: 'none', borderRadius: 1, boxShadow: 'none' }}>Lập Phiếu Kiểm</Button>
            <Button size="small" variant="contained" startIcon={<BalanceIcon />} sx={{ bgcolor: '#f39c12', '&:hover': { bgcolor: '#db8b0b' }, textTransform: 'none', borderRadius: 1, boxShadow: 'none' }}>Cân Bằng Kho</Button>
            ...
            <Table sx={{ minWidth: 1000 }}>
              <TableHead sx={{ bgcolor: '#ffffff' }}>
                <TableRow>
                  ...
                  {['Mã Phiếu', 'Ngày Kiểm', 'Cửa Hàng / Kho', 'Tổng SL Lệch', 'Tổng Giá Trị Lệch', 'Người Kiểm', 'Trạng Thái'].map((col) => (
              ...
              <TableBody>
                {mockChecks.map((row) => (
                  <TableRow key={row.no} hover sx={{ '&:last-child td': { border: 0 } }}>
                    ...
                    <TableCell sx={{ borderBottom: '1px solid #f1f5f9', fontSize: '0.85rem', color: '#475569', p: 1.5 }}>{row.ngayKiem}</TableCell>
                    <TableCell sx={{ borderBottom: '1px solid #f1f5f9', fontSize: '0.85rem', color: '#0f172a', fontWeight: 600, p: 1.5 }}>{row.kho}</TableCell>
                    
                    <TableCell sx={{ borderBottom: '1px solid #f1f5f9', fontSize: '0.85rem', fontWeight: 700, color: row.slLech < 0 ? '#dc2626' : '#16a34a', p: 1.5 }}>
                      {row.slLech > 0 ? '+' : ''}{row.slLech}
                    </TableCell>
                    <TableCell sx={{ borderBottom: '1px solid #f1f5f9', fontSize: '0.85rem', fontWeight: 700, color: row.giaTriLech < 0 ? '#dc2626' : '#16a34a', p: 1.5 }}>
                      {row.giaTriLech > 0 ? '+' : ''}{formatCurrency(row.giaTriLech)}
                    </TableCell>
                    
                    <TableCell sx={{ borderBottom: '1px solid #f1f5f9', fontSize: '0.85rem', color: '#475569', p: 1.5 }}>{row.nguoiKiem}</TableCell>
                    <TableCell sx={{ borderBottom: '1px solid #f1f5f9', p: 1.5 }}>
                      {row.trangThai === 'Đã cân bằng' ? 
          ...
          <Box sx={{ p: 1.5, bgcolor: '#ffffff', borderTop: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
             ...
             <Typography variant="body2" color="text.secondary" sx={{ fontWeight: 500 }}>1 - {mockChecks.length} of {mockChecks.length} items</Typography>
          </Box>
        </CardContent>
      </Card>
    </Box>
